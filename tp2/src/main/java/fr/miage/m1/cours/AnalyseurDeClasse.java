...
public class AnalyseurDeClasse {
    ...
    private void afficheAttributs() throws ClassNotFoundException {
        ...
        for (Field field : champs) {
            ...
            System.out.println("  " + java.lang.reflect.Modifier.toString(modif) + " " + field.getType() + " "
                    + field.getName() + ";");
        }
    }
    ...
    private void afficheMethodes() throws ClassNotFoundException {
        System.out.println("\n// Methodes");
        Method[] methodes = c.getDeclaredMethods();
        for (Method m : methodes) {
            int modif = m.getModifiers();
            System.out.print("  " + java.lang.reflect.Modifier.toString(modif) + " " + m.getReturnType() + " "
                    + m.getName() + " (");
            Class<?>[] parametres = m.getParameterTypes();
            int testindice = parametres.length - 1;
            for (int i = 0; i < parametres.length; i++) {
                System.out.print(parametres[i].getName());
                if (i < testindice) {
                    System.out.print(",");
                }
            }
